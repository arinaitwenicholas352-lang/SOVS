...
function useDismiss(context, props = {}) {
  ...
  const pressStartedInsideRef = React7.useRef(false);
  const pressStartPreventedRef = React7.useRef(false);
  const suppressNextOutsideClickRef = React7.useRef(false);
  const {
...
function useInteractions(propsList = []) {
  ...
  const getTriggerProps = React14.useCallback(
...
function useListNavigation(context, props) {
  ...
  const previousMountedRef = React15.useRef(!!floatingElement);
  ...
  const cancelQueuedFocusRef = React15.useRef(null);
...
function useTypeahead(context, props) {
  ...
  const stringRef = React16.useRef("");
  const prevIndexRef = React16.useRef(selectedIndex ?? activeIndex ?? -1);
  const matchIndexRef = React16.useRef(null);
  useIsoLayoutEffect(() => {
...
function useFloating2(options = {}) {
  ...
  const [localFloatingElement, setLocalFloatingElement] = React19.useState(null);
...
function useEnhancedClickHandler(handler) {
  const lastClickInteractionTypeRef = React25.useRef("");
  const handlePointerDown = React25.useCallback((event) => {
  ...
  const handleClick = React25.useCallback((event) => {
    if (event.detail === 0) {
...
function useOpenInteractionType(open) {
  const [openMethod, setOpenMethod] = React26.useState(null);
...
var InternalBackdrop = React27.forwardRef(function InternalBackdrop2(props, ref) {
  const {
    cutout,
    ...otherProps
  } = props;
...
//# sourceMappingURL=chunk-FUZPO62Q.js.map
